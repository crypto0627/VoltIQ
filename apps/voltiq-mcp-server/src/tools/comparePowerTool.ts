...
export function registerComparePowerUsageTool(
  ...
  server.tool(
    ...
    {
      comparisonType: z.enum(["month", "date", "timeRange"]).describe("比較類型：month(月份)、date(日期)或timeRange(時間區間)"),
      firstPeriod: z.string().describe("第一個時間段，根據comparisonType格式不同：\n- month: 'MM' (例如 '10')\n- date: 'MM/DD' (例如 '10/01')\n- timeRange: 'MM/DD HH:mm-HH:mm' (例如 '10/01 08:00-10:00')"),
      secondPeriod: z.string().describe("第二個時間段，格式與firstPeriod相同"),
    },
    async (input: { comparisonType: "month" | "date" | "timeRange"; firstPeriod: string; secondPeriod: string }) => {
      try {
        ...
        const parseTimeRange = (period: string) => {
          ...
          return { date: period, startTime: "00:00", endTime: "23:59" };
        };
        ...
        // 獲取兩個時間段的數據
        const getDataForPeriod = async (range: { date: string; startTime: string; endTime: string }) => {
          let result;
          ...
          if (comparisonType === "month") {
            ...
            // 合併該月份所有日期的數據
            const allUsageData = monthDocs.flatMap(doc => doc.usageData);
            const usageInRange = allUsageData.filter(entry => 
            ...
            // 處理日期和時間區間比較
            result = await collection.findOne<PowerUsageData>({ date: range.date });
            if (!result) return null;

            const usageInRange = result.usageData.filter((entry) => {
              return entry.time >= range.startTime && entry.time <= range.endTime;
            });
        ...
        const generateComparisonText = () => {
          ...
          const difference = Math.round((secondData.totalUsage - firstData.totalUsage) * 100) / 100;
          const percentageChange = Math.round((difference / firstData.totalUsage) * 10000) / 100;
        ...
        const chartData = [
          {
            period: comparisonType === "month" ? `${firstRange.date}月` : firstRange.date,
            usage: firstData.totalUsage
          },
          {
            period: comparisonType === "month" ? `${secondRange.date}月` : secondRange.date,
            usage: secondData.totalUsage
          }
        ];
