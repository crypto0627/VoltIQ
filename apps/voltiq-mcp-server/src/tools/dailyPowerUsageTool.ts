...
export function registerDailyPowerUsageTool(server: McpServer, collection: Collection) {
  server.tool(
    ...
    async (input: { date: string; startTime: string; endTime: string }) => {
      try {
        ...
        console.error("Error during 'get_power_usage_by_time_range' execution:", error);
        return {
          content: [
            {
              type: "text",
