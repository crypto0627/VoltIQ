...
export async function POST(request: Request) {
  try {
    ...
    if (dataToVectorize.length === 0) {
      return NextResponse.json({ message: "No data found in MongoDB collection." }, { status: 200 });
    }
    ...
    return NextResponse.json({ message: "Data successfully vectorized and stored in ChromaDB." }, { status: 200 });
