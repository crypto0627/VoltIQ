...
export const useUsageCalculations = () => {
  ...
  const calculateUsage = useMemo(() => {
    if (!data.length || currentTimeIndex === 0) return {
      peakUsage: 0,
      offPeakUsage: 0,
      midPeakUsage: 0,
      totalCost: 0,
    ...
    for (let i = 1; i <= currentTimeIndex; i++) {
      ...
      if (currTime <= "09:00" || (prevTime >= "11:00" && currTime <= "14:00")) {
        offPeakSum += (data[i-1].powerUsage + data[i].powerUsage) * 0.25 / 2;
      }
      
      // Mid-peak hours (09:00-16:00, 22:00-24:00)
      if ((prevTime >= "09:00" && currTime <= "16:00") || (prevTime >= "22:00" && currTime <= "23:30")) {
        midPeakSum += (data[i-1].powerUsage + data[i].powerUsage) * 0.25 / 2;
      }
    ...
    const totalCost = Math.round(offPeakSum * 1.58 + midPeakSum * 3.63 + peakSum * 5.8);
